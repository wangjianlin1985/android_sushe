...
public class NoticeAction extends BaseAction {
    ...
    public String QueryNotice() {
        ...
        List<Notice> noticeList = noticeDAO.QueryNoticeInfo(noticeTitle, currentPage);
        /*计算总的页数和总的记录数*/
        noticeDAO.CalculateTotalPageAndRecordNumber(noticeTitle);
        /*获取到总的页码数目*/
        ...
        ctx.put("noticeTitle", noticeTitle);
        return "query_view";
    }
    ...
    public String QueryNoticeOutputToExcel() { 
        if(noticeTitle == null) noticeTitle = "";
        List<Notice> noticeList = noticeDAO.QueryNoticeInfo(noticeTitle);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "Notice信息记录"; 
        String[] headers = { "公告id","公告标题","公告时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<noticeList.size();i++) {
        	Notice notice = noticeList.get(i); 
        	dataset.add(new String[]{notice.getNoticeId() + "",notice.getNoticeTitle(),notice.getNoticeTime()});
        }
        /*
