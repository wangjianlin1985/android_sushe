...
public class RoomEditActivity extends Activity {
			...
			public void onClick(View v) {
				try {
					...
					if(ET_roomName.getText().toString().equals("")) {
						Toast.makeText(RoomEditActivity.this, "房间名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_bedNum.getText().toString().equals("")) {
						Toast.makeText(RoomEditActivity.this, "床位数输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传房间信息信息*/
					RoomEditActivity.this.setTitle("正在更新房间信息信息，稍等...");
	...
	private void initViewData() {
		...
		for (int i = 0; i < houseList.size(); i++) {
			if (room.getHouseObj() == houseList.get(i).getHouseId()) {
				this.spinner_houseObj.setSelection(i);
				break;
			}
		}
		this.ET_roomName.setText(room.getRoomName());
		this.ET_bedNum.setText(room.getBedNum() + "");
	}
	@Override
