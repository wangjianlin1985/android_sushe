			...
			public void onClick(View v) {
				try {
					...
					if(ET_repaitTitle.getText().toString().equals("")) {
						Toast.makeText(RepairAddActivity.this, "故障简述输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_repairContent.getText().toString().equals("")) {
						Toast.makeText(RepairAddActivity.this, "故障详述输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_handleResult.getText().toString().equals("")) {
						Toast.makeText(RepairAddActivity.this, "处理结果输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传报修信息信息*/
					RepairAddActivity.this.setTitle("正在上传报修信息信息，稍等...");
