...
/*需要绑定的数据*/
  ...
  public View getView(int position, View convertView, ViewGroup parent) { 
	  ...
	  holder.tv_houseObj = (TextView)convertView.findViewById(R.id.tv_houseObj);
	  ...
	  /*设置各个控件的展示内容*/
	  holder.tv_roomId.setText("房间id：" + mData.get(position).get("roomId").toString());
	  holder.tv_houseObj.setText("所在宿舍：" + (new HouseService()).GetHouse(Integer.parseInt(mData.get(position).get("houseObj").toString())).getHouseName());
	  holder.tv_roomName.setText("房间名称：" + mData.get(position).get("roomName").toString());
	  holder.tv_bedNum.setText("床位数：" + mData.get(position).get("bedNum").toString());
	  /*返回修改好的view*/
	  return convertView; 
    } 
